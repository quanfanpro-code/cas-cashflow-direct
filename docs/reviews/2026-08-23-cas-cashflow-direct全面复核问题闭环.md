...
> 当前状态：阶段4自动验收和阶段5独立代码复核、真实输入门禁冒烟、真实状态结构工作簿冒烟已完成；旧真实案例结果被非退化门禁拒绝，真实业务分类验收仍待有效摘要语义输入。本结论允许交付代码与测试修复，但不宣称取得新的真实案例验收通过。
...
- 最终执行`python -X utf8 -m pytest -q`：`616 passed, 2 skipped, 93 subtests passed`，0失败；两个跳过项分别是未设置真实验收目录、未设置两份等价明细的比较参数；
- 四项严重错误定向测试、相关工作簿/流程测试及全量回归均为0失败；
- 真实验收测试已修正数据区边界：按`review_batches`确定可见业务行，不再把隐藏候选区造成的`max_row=312`误判为312个业务行；定向结果为`29 passed, 2 skipped, 13 subtests passed`；
...
- 当前提交已在`C:\Users\27651\Desktop\真实测试案例\真实样本冒烟_3dd8ff4_20260823_170859\运行_20260823_170943_8ca399ce`重新读取两份真实输入，完成字段映射、3项一级科目映射、13项现金范围纳入、5项排除、科目语义导入和5,355项摘要任务生成；随后按上述非退化门禁停止，未生成工作簿。运行前后输入SHA-256分别保持`B644A9E6...D53F0`和`231799DF...392B`不变。
- 为核验四项工作簿写出行为，另在`C:\Users\27651\Desktop\真实测试案例\真实样本冒烟_3dd8ff4_20260823_170859\结构工作簿_复用既有真实状态`隔离复制旧状态并由当前写出层重建工作簿。111个可见业务行的U、AK、AL全部通过；差异表与当前决定生成器逐行一致，唯一一行是内部划转并显示“评分不适用”。该工作簿复用旧业务状态，明确不作为真实分类验收结果。
- 本次四项修复、自动回归和两层冒烟没有采用任何模型输出，没有生成新的摘要语义结果，也没有覆盖旧工作簿；旧工作簿SHA-256仍为`688A22ED...EC22`。真实案例验收状态明确为“待完成”。
- 当前阶段5结论包括：代码独立复核通过、真实输入流程能正确停在非退化门禁、四项工作簿写出在真实状态结构冒烟中通过、旧错误结论已撤销。后续如提供有效摘要语义结果，仍须另行重跑并核对70/90分、自动修改、待人工、正表核对报告和差异明细。
